...
static inline SlValue sl_str(const char* s) { SlValue v; v.type = SL_STR; v.sval = strdup(s); return v; }
...
static inline const char* sl_to_str(SlValue v) {
...
static inline SlArray* sl_arr_new_i32(int initial_cap) {
    if (initial_cap < 4) initial_cap = 4;
    SlArray* a = (SlArray*)malloc(sizeof(SlArray));
    a->encoding = SL_ENC_I32;
    a->i32 = (int*)malloc((size_t)initial_cap * sizeof(int));
    a->len = 0;
    a->cap = initial_cap;
    a->refcount = 1;
    return a;
...
static inline SlArray* sl_arr_new_i64(int initial_cap) {
    SlArray* a = (SlArray*)malloc(sizeof(SlArray));
    a->encoding = SL_ENC_I64;
    a->i64 = (long long*)malloc(initial_cap * 8);
    a->len = 0;
    a->cap = initial_cap;
    a->refcount = 1;
    return a;
...
static inline long long sl_arr_get_i64(SlArray* a, int i) {
    return a->i64[i];
}

static inline void sl_arr_set_i64(SlArray* a, int i, long long val) {
    a->i64[i] = val;
}
...
static inline void sl_arr_push_i64(SlArray* a, long long val) {
    if (a->encoding != SL_ENC_I64) {
        sl_arr_push_int(a, val);
        return;
    }
    if (SL_LIKELY(a->len < a->cap)) {
        a->i64[a->len++] = val;
        return;
    }
    sl_arr_grow(a);
...
static void sl_arr_reserve(SlArray* a, int min_cap) {
    if (!a || min_cap <= 0 || a->cap >= min_cap) return;
    a->cap = min_cap;
    switch (a->encoding) {
        case SL_ENC_U8: a->u8 = (unsigned char*)realloc(a->u8, (size_t)a->cap); break;
        case SL_ENC_U16: a->u16 = (unsigned short*)realloc(a->u16, (size_t)a->cap * 2); break;
        case SL_ENC_I32: a->i32 = (int*)realloc(a->i32, (size_t)a->cap * sizeof(int)); break;
        case SL_ENC_I64: a->i64 = (long long*)realloc(a->i64, (size_t)a->cap * sizeof(long long)); break;
        case SL_ENC_F64: a->f64 = (double*)realloc(a->f64, (size_t)a->cap * sizeof(double)); break;
        case SL_ENC_MIXED: a->data = (SlValue*)realloc(a->data, (size_t)a->cap * sizeof(SlValue)); break;
    }
}
...
static char* sl_strcat_fr(const char* a, const char* b) {
    int la = (int)strlen(a), lb = (int)strlen(b);
    char* r = (char*)malloc(la + lb + 1);
    memcpy(r, a, la);
    memcpy(r + la, b, lb + 1);
...
static char* sl_strcat_fl(const char* a, const char* b) {
    int la = (int)strlen(a), lb = (int)strlen(b);
    char* r = (char*)malloc(la + lb + 1);
    memcpy(r, a, la);
    memcpy(r + la, b, lb + 1);
...
static char* sl_strcat3(const char* a, const char* b, const char* c) {
...
static char* sl_strcat3_fl(const char* a, const char* b, const char* c) {
    int la = (int)strlen(a), lb = (int)strlen(b), lc = (int)strlen(c);
    char* r = (char*)malloc(la + lb + lc + 1);
    memcpy(r, a, la);
    memcpy(r + la, b, lb);
...
static char* sl_strcat3_fr(const char* a, const char* b, const char* c) {
    int la = (int)strlen(a), lb = (int)strlen(b), lc = (int)strlen(c);
    char* r = (char*)malloc(la + lb + lc + 1);
    memcpy(r, a, la);
    memcpy(r + la, b, lb);
...
typedef struct { char* data; int len; int cap; } SlStrBuf;

static SlStrBuf sl_sb_new(const char* init) {
    int l = (int)strlen(init);
    int c = l * 2 + 32;
    SlStrBuf sb; sb.data = (char*)malloc(c); sb.len = l; sb.cap = c;
    memcpy(sb.data, init, l + 1);
    return sb;
}

static inline void sl_sb_ensure(SlStrBuf* sb, int need) {
    if (need >= sb->cap) {
        sb->cap = need + need / 2 + 32;
        sb->data = (char*)realloc(sb->data, sb->cap);
    }
}

static inline void sl_sb_append(SlStrBuf* sb, const char* b, int blen) {
    if (sb->len + blen + 1 >= sb->cap) {
        sb->cap = (sb->len + blen) * 2 + 32;
        sb->data = (char*)realloc(sb->data, sb->cap);
    }
    memcpy(sb->data + sb->len, b, blen + 1);
    sb->len += blen;
}

static inline void sl_sb_append_c(SlStrBuf* sb, char c) {
    if (sb->len + 2 >= sb->cap) {
        sb->cap = sb->cap * 2 + 32;
        sb->data = (char*)realloc(sb->data, sb->cap);
    }
...
static char* sl_sb_to_str(SlStrBuf* sb) {
    sb->data[sb->len] = '\0';
    return sb->data;
}

static char* sl_strcat_inplace(char* a, const char* b) {
    int la = (int)strlen(a), lb = (int)strlen(b);
    int newLen = la + lb;
    int newCap = newLen + newLen / 2 + 16;
    char* r = (char*)realloc(a, newCap);
    memcpy(r + la, b, lb + 1);
    return r;
}

static char* sl_strcat_inplace_fr(char* a, const char* b) {
    int la = (int)strlen(a), lb = (int)strlen(b);
    int newLen = la + lb;
    int newCap = newLen + newLen / 2 + 16;
    char* r = (char*)realloc(a, newCap);
...
static char* sl_arr_join(SlArray* a, const char* sep) {
    ...
    int total = 256 * a->len + seplen * (a->len - 1) + 1;
    char* r = (char*)malloc(total);
    int pos = 0;
    for (int i = 0; i < a->len; i++) {
        if (i > 0) { memcpy(r + pos, sep, seplen); pos += seplen; }
        SlValue v = sl_arr_getval(a, i);
        if (v.type == SL_STR) {
            pos += snprintf(r + pos, total - pos, "%s", v.sval);
        } else if (v.type == SL_DBL) {
            pos += snprintf(r + pos, total - pos, "%f", v.dval);
        } else {
...
static SlValue sl_v_add(SlValue a, SlValue b) {
    if (a.type == SL_INT && b.type == SL_INT) { a.ival += b.ival; return a; }
    if (a.type == SL_STR || b.type == SL_STR) {
        const char* sa = sl_to_str(a); const char* sb = sl_to_str(b);
        if (!sa) sa = "null"; if (!sb) sb = "null";
        SlValue r; r.type = SL_STR; r.sval = sl_strcat(sa, sb); return r;
    }
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_dbl(sl_to_dbl(a) + sl_to_dbl(b));
    return sl_int(sl_to_int(a) + sl_to_int(b));
}

static SlValue sl_v_sub(SlValue a, SlValue b) {
    if (a.type == SL_INT && b.type == SL_INT) { a.ival -= b.ival; return a; }
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_dbl(sl_to_dbl(a) - sl_to_dbl(b));
    return sl_int(sl_to_int(a) - sl_to_int(b));
}

static SlValue sl_v_mul(SlValue a, SlValue b) {
    if (a.type == SL_INT && b.type == SL_INT) { a.ival *= b.ival; return a; }
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_dbl(sl_to_dbl(a) * sl_to_dbl(b));
    return sl_int(sl_to_int(a) * sl_to_int(b));
}

static SlValue sl_v_div(SlValue a, SlValue b) {
    if (a.type == SL_INT && b.type == SL_INT) { if (b.ival == 0) b.ival = 1; a.ival /= b.ival; return a; }
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_dbl(sl_to_dbl(a) / sl_to_dbl(b));
    long long bi = sl_to_int(b); if (bi == 0) bi = 1;
    return sl_int(sl_to_int(a) / bi);
}

static SlValue sl_v_mod(SlValue a, SlValue b) {
    if (a.type == SL_INT && b.type == SL_INT) { if (b.ival == 0) b.ival = 1; a.ival %= b.ival; return a; }
    long long bi = sl_to_int(b); if (bi == 0) bi = 1;
    return sl_int(sl_to_int(a) % bi);
}

static SlValue sl_v_bitAnd(SlValue a, SlValue b) { return sl_int(sl_to_int(a) & sl_to_int(b)); }
static SlValue sl_v_bitOr(SlValue a, SlValue b) { return sl_int(sl_to_int(a) | sl_to_int(b)); }
static SlValue sl_v_bitXor(SlValue a, SlValue b) { return sl_int(sl_to_int(a) ^ sl_to_int(b)); }
static SlValue sl_v_bitNot(SlValue a) { return sl_int(~sl_to_int(a)); }
static SlValue sl_v_shl(SlValue a, SlValue b) { return sl_int(sl_to_int(a) << sl_to_int(b)); }
static SlValue sl_v_shr(SlValue a, SlValue b) { return sl_int(sl_to_int(a) >> sl_to_int(b)); }
static SlValue sl_v_ushr(SlValue a, SlValue b) { return sl_int((unsigned long long)sl_to_int(a) >> sl_to_int(b)); }

static SlValue sl_v_neg(SlValue a) {
    if (a.type == SL_DBL) return sl_dbl(-a.dval);
    return sl_int(-sl_to_int(a));
}

static SlValue sl_v_eq(SlValue a, SlValue b) {
    if (a.type == SL_STR && b.type == SL_STR) return sl_bool(sl_str_eq(a.sval, b.sval));
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_bool(sl_to_dbl(a) == sl_to_dbl(b));
    if (a.type == b.type) return sl_bool(sl_to_int(a) == sl_to_int(b));
    return sl_bool(0);
}

static SlValue sl_v_ne(SlValue a, SlValue b) {
    SlValue eq = sl_v_eq(a, b); return sl_bool(!sl_to_bool(eq));
}

static SlValue sl_v_lt(SlValue a, SlValue b) {
    if (a.type == SL_STR && b.type == SL_STR) return sl_bool(sl_str_lt(a.sval, b.sval));
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_bool(sl_to_dbl(a) < sl_to_dbl(b));
    return sl_bool(sl_to_int(a) < sl_to_int(b));
}

static SlValue sl_v_gt(SlValue a, SlValue b) {
    if (a.type == SL_STR && b.type == SL_STR) return sl_bool(sl_str_gt(a.sval, b.sval));
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_bool(sl_to_dbl(a) > sl_to_dbl(b));
    return sl_bool(sl_to_int(a) > sl_to_int(b));
}

static SlValue sl_v_le(SlValue a, SlValue b) {
    if (a.type == SL_STR && b.type == SL_STR) return sl_bool(sl_str_le(a.sval, b.sval));
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_bool(sl_to_dbl(a) <= sl_to_dbl(b));
    return sl_bool(sl_to_int(a) <= sl_to_int(b));
}

static SlValue sl_v_ge(SlValue a, SlValue b) {
    if (a.type == SL_STR && b.type == SL_STR) return sl_bool(sl_str_ge(a.sval, b.sval));
    if (a.type == SL_DBL || b.type == SL_DBL) return sl_bool(sl_to_dbl(a) >= sl_to_dbl(b));
    return sl_bool(sl_to_int(a) >= sl_to_int(b));
}

static SlValue sl_v_not(SlValue a) { return sl_bool(!sl_to_bool(a)); }

static SlValue sl_v_and(SlValue a, SlValue b) { return sl_bool(sl_to_bool(a) && sl_to_bool(b)); }

static SlValue sl_v_or(SlValue a, SlValue b) { return sl_bool(sl_to_bool(a) || sl_to_bool(b)); }

static SlValue sl_v_len(SlValue a) {
    if (a.type == SL_ARR && a.aval) return sl_int(sl_arr_len(a.aval));
    if (a.type == SL_STR && a.sval) return sl_int((long long)strlen(a.sval));
    return sl_int(0);
}

static SlValue sl_v_push(SlValue arr, SlValue val) {
    if (arr.type == SL_ARR && arr.aval) sl_arr_push(arr.aval, val);
    return arr;
}

static SlValue sl_v_index(SlValue obj, SlValue key) {
    if (obj.type == SL_ARR && obj.aval) {
        long long idx = sl_to_int(key);
        if (idx >= 0 && idx < obj.aval->len) return sl_arr_getval(obj.aval, (int)idx);
        return sl_null();
    }
    if (obj.type == SL_MAP && obj.mval) {
        if (key.type == SL_STR && key.sval) return sl_map_get(obj.mval, key.sval, sl_null());
        char kbuf[32]; snprintf(kbuf, sizeof(kbuf), "%lld", sl_to_int(key));
        return sl_map_get(obj.mval, kbuf, sl_null());
    }
    return sl_null();
}

static void sl_v_set_index(SlValue obj, SlValue key, SlValue val) {
    if (obj.type == SL_MAP && obj.mval) {
        if (key.type == SL_STR && key.sval) { sl_map_set(obj.mval, key.sval, val); return; }
        char kbuf[32]; snprintf(kbuf, sizeof(kbuf), "%lld", sl_to_int(key));
        sl_map_set(obj.mval, kbuf, val); return;
    }
}

static SlValue sl_v_member(SlValue obj, const char* prop) {
    if (obj.type == SL_ARR && obj.aval) {
        if (strcmp(prop, "length") == 0) return sl_int(sl_arr_len(obj.aval));
    }
    if (obj.type == SL_STR && obj.sval) {
        if (strcmp(prop, "length") == 0) return sl_int((long long)strlen(obj.sval));
    }
    if (obj.type == SL_MAP && obj.mval) {
        return sl_map_get(obj.mval, prop, sl_null());
    }
    return sl_null();
}

static void sl_v_set_member(SlValue obj, const char* prop, SlValue val) {
    if (obj.type == SL_MAP && obj.mval) {
        sl_map_set(obj.mval, prop, val);
    }
}

static SlValue sl_v_print(SlValue a) {
    if (a.type == SL_NULL) printf("null\n");
    else if (a.type == SL_BOOL) printf("%s\n", a.ival ? "true" : "false");
    else if (a.type == SL_INT) printf("%lld\n", a.ival);
    else if (a.type == SL_DBL) printf("%f\n", a.dval);
    else if (a.type == SL_STR && a.sval) printf("%s\n", a.sval);
    else if (a.type == SL_ARR && a.aval) printf("[array len=%d]\n", a.aval->len);
    else if (a.type == SL_MAP && a.mval) printf("{map size=%d}\n", a.mval->len);
    else printf("(unknown)\n");
    return sl_null();
}

static SlValue sl_v_mapSet(SlValue m, SlValue k, SlValue v) {
    if (m.type == SL_MAP && m.mval && k.type == SL_STR && k.sval) {
        sl_map_set(m.mval, k.sval, v);
    }
    return m;
}

static SlValue sl_v_mapGet(SlValue m, SlValue k) {
    if (m.type == SL_MAP && m.mval && k.type == SL_STR && k.sval) {
        return sl_map_get(m.mval, k.sval, sl_null());
    }
    return sl_null();
}

static SlValue sl_v_mapHas(SlValue m, SlValue k) {
    if (m.type == SL_MAP && m.mval && k.type == SL_STR && k.sval) {
        return sl_bool(sl_map_has(m.mval, k.sval));
    }
    return sl_bool(0);
}

static SlValue sl_v_mapKeys(SlValue m) {
    if (m.type == SL_MAP && m.mval) return sl_box_arr(sl_map_keys(m.mval));
    return sl_box_arr(sl_arr_new(1));
}

static SlValue sl_v_mapValues(SlValue m) {
    if (m.type == SL_MAP && m.mval) return sl_box_arr(sl_map_values(m.mval));
    return sl_box_arr(sl_arr_new(1));
}

static SlValue sl_v_mapSize(SlValue m) {
    if (m.type == SL_MAP && m.mval) return sl_int(sl_map_size(m.mval));
    return sl_int(0);
}

static SlValue sl_v_setAdd(SlValue s, SlValue v) {
    if (s.type == SL_PTR && s.pval) sl_set_add((SlSet*)s.pval, v);
    return s;
}

static SlValue sl_v_setHas(SlValue s, SlValue v) {
    if (s.type == SL_PTR && s.pval) return sl_bool(sl_set_has((SlSet*)s.pval, v));
    return sl_bool(0);
}

static SlValue sl_v_setSize(SlValue s) {
    if (s.type == SL_PTR && s.pval) return sl_int(sl_set_size((SlSet*)s.pval));
    return sl_int(0);
}

static SlValue sl_v_setToArray(SlValue s) {
    if (s.type == SL_PTR && s.pval) return sl_box_arr(sl_set_toArray((SlSet*)s.pval));
    return sl_box_arr(sl_arr_new(1));
}

static SlValue sl_v_charAt(SlValue s, SlValue idx) {
    if (s.type == SL_STR && s.sval) return sl_str(sl_str_charAt(s.sval, sl_to_int(idx)));
    return sl_str("");
}

static SlValue sl_v_substring(SlValue s, SlValue start, SlValue end) {
    if (s.type == SL_STR && s.sval) return sl_str(sl_str_substring(s.sval, sl_to_int(start), sl_to_int(end)));
    return sl_str("");
}

static SlValue sl_v_includes(SlValue a, SlValue b) {
    if (a.type == SL_STR && a.sval && b.type == SL_STR && b.sval) {
        return sl_bool(strstr(a.sval, b.sval) != NULL ? 1 : 0);
    }
    if (a.type == SL_ARR && a.aval) {
        for (int i = 0; i < a.aval->len; i++) {
            SlValue v = sl_arr_getval(a.aval, i);
            if (sl_to_bool(sl_v_eq(v, b))) return sl_bool(1);
        }
    }
    return sl_bool(0);
}

static SlValue sl_v_codePointAt(SlValue s, SlValue idx) {
    if (s.type == SL_STR && s.sval) return sl_int(sl_codePointAt(s.sval, sl_to_int(idx)));
    return sl_int(-1);
}

static SlValue sl_v_readFile(SlValue path) {
    if (path.type == SL_STR && path.sval) return sl_str(sl_readFile(path.sval));
    return sl_str("");
}

static SlValue sl_v_writeFile(SlValue path, SlValue content) {
    if (path.type == SL_STR && path.sval && content.type == SL_STR && content.sval) {
        return sl_int(sl_writeFile(path.sval, content.sval));
    }
    return sl_int(0);
}

static SlValue sl_v_join(SlValue arr, SlValue sep) {
    if (arr.type == SL_ARR && arr.aval && sep.type == SL_STR && sep.sval) {
        return sl_str(sl_arr_join(arr.aval, sep.sval));
    }
    return sl_str("");
}

static SlValue sl_v_split(SlValue s, SlValue sep) {
    if (s.type == SL_STR && s.sval && sep.type == SL_STR && sep.sval) {
        return sl_box_arr(sl_str_split(s.sval, sep.sval));
    }
    return sl_box_arr(sl_arr_new(1));
}

static SlValue sl_v_args(int argc, char** argv) {
    return sl_box_arr(sl_args(argc, argv));
}

static SlValue sl_v_map_new() {
    return sl_map(sl_map_new(8));
}

static SlValue sl_v_set_new() {
    SlSet* s = sl_set_new();
    SlValue v; v.type = SL_PTR; v.pval = s; return v;
}

static SlValue sl_v_arr_new() {
    return sl_box_arr(sl_arr_new(4));
}

static SlValue sl_v_arr_new_i64(int initial_cap) {
    return sl_box_arr(sl_arr_new_i64(initial_cap));
}

static SlValue sl_v_arr_new_i32(int initial_cap) {
    return sl_box_arr(sl_arr_new_i32(initial_cap));
}

static SlValue sl_v_upper(SlValue s) {
    if (s.type == SL_STR && s.sval) return sl_str(sl_str_upper(s.sval));
    return sl_str("");
}

static SlValue sl_v_lower(SlValue s) {
    if (s.type == SL_STR && s.sval) return sl_str(sl_str_lower(s.sval));
    return sl_str("");
}

static SlValue sl_v_trim(SlValue s) {
    if (s.type == SL_STR && s.sval) return sl_str(sl_str_trim(s.sval));
    return sl_str("");
}

static SlValue sl_v_toString(SlValue v) {
    char buf[64];
    if (v.type == SL_NULL) return sl_str("null");
    if (v.type == SL_BOOL) return sl_str(v.ival ? "true" : "false");
    if (v.type == SL_INT) { snprintf(buf, sizeof(buf), "%lld", v.ival); return sl_str(strdup(buf)); }
    if (v.type == SL_DBL) { snprintf(buf, sizeof(buf), "%f", v.dval); return sl_str(strdup(buf)); }
    if (v.type == SL_STR) return sl_str(strdup(v.sval));
    return sl_str("(value)");
}
