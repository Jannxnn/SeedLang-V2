...
**主仓库（对外请用完整 HTTPS，避免只写 `org/repo` 或被客户端裁成半截）**：[https://github.com/seedlang-team/seedlang](https://github.com/seedlang-team/seedlang)（与 `package.json` 的 `repository` 一致）。若你克隆的是 fork，请在该仓库 GitHub 页的 **Code → HTTPS** 复制地址，替换下文 `git clone` 中的 URL。
...
# 运行 .seed 文件（最小示例 examples/hello/；CLC 语料见 examples/clc/）
...
**输入 / 输出（怎么接数据）**：标准输出用 **`print(...)`**；从磁盘读入用 **`readFile("相对或绝对路径")`**，写出用 **`writeFile(path, text)`**（与规范 §8 一致）。仓库示例：`node dist/cli.js examples/hello/io_read_file.seed`。Node 宿主另有异步一行 **`input(可选提示)`**（stdin）；CLI 会 **`await` 顶层语句返回的 Promise**，但 **`async fn` 内部的 `await input`** 仍受解释器异步体实现所限，**教程与可靠脚本优先用文件 IO**；细节见 **[docs/AI_QUICK_START.md](docs/AI_QUICK_START.md)**（文中「输入与输出（IO）」一节）。

更长的语法片段与「给 AI 看的」写法见 **[docs/AI_QUICK_START.md](docs/AI_QUICK_START.md)**、**[docs/AI_PROMPT.md](docs/AI_PROMPT.md)**（与 README 互补：README 偏安装与后端，这两份偏语法与提示词）。
...
2. `node dist/cli.js examples/hello/hello.seed`（最小示例；也可把上面的 `print("Hello World")` 存成任意 `.seed` 再跑同命令）
3. `node dist/cli.js --compile your.seed -o out.js`，再 `node out.js`（产物结构随示例变化，接进严肃项目前请先读生成代码）
...
**宏（编译期）**：`macro name(params) { … }`，调用 `name!(args)`。宏体里**新引入**的绑定会重命名，避免和调用方作用域撞名；**形参**代入调用处的实参，可读写调用方变量（规范里称为「部分卫生」）。详见 [LANGUAGE_SPEC_REFACTOR_DRAFT.md §7.4](docs/LANGUAGE_SPEC_REFACTOR_DRAFT.md#74-macro-system)。
...
│   └── win32/                    # Win32 性能测试
├── examples/                     # `hello/` 最小脚本 + CLC / Win32（见 examples/README.md）
...
| 文档 | 说明 |
|------|------|
| [docs/AI_QUICK_START.md](docs/AI_QUICK_START.md) | Hello World、`print`、常用语法速查（人类与 AI 辅助） |
| [docs/SYNTAX_MENTAL_MODEL.md](docs/SYNTAX_MENTAL_MODEL.md) | 为何有人觉得语法「杂」、推荐单一写法与对外口径 |
| [docs/AI_PROMPT.md](docs/AI_PROMPT.md) | 给模型用的 SeedLang 语法与约定 |
