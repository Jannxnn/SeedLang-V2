    ...
    for (int i = 0; i < src->len; i++) {
        ...
        for (int j = 0; j < dst->len; j++) {
            if (strcmp(dst->keys[j], src->keys[i]) == 0) {
                dst->vals[j] = src->vals[i];
                found = 1;
                break;
            }
        }
        if (!found) sl_map_set(dst, src->keys[i], src->vals[i]);
    }
}
...
long long sl_ly;
long long sl_direct;
long long sl_clamp_user(long long sl_v, long long sl_lo, long long sl_hi) {
    if (sl_v < sl_lo) {
        return sl_lo;
    }
    if (sl_v > sl_hi) {
...
SlArray* sl_makeGrid(long long sl_h, long long sl_w, long long sl_init) {
    long long sl_y = 0;
    long long sl_x = 0;
    SlArray* sl_g = 0;
    SlArray* sl_row = 0;
    sl_release_arr(sl_g); sl_g = sl_arr_new(16);

    while (sl_y < sl_h) {
        sl_release_arr(sl_row); sl_row = sl_arr_new(16);
        sl_x = 0;
        while (sl_x < sl_w) {
            (sl_arr_push_int(sl_row, sl_init), 0);
            sl_x++;
        }
        (sl_arr_push(sl_g, sl_box_arr(sl_row)), 0);
        sl_y++;
    }
    SlArray* _r = sl_g;
    sl_release_arr(sl_row);
    return _r;
}
SlMap* sl_buildScene() {
    long long sl_x = 0;
    ...
    SlArray* sl_occ = 0;
    SlArray* sl_emi = 0;
    sl_release_arr(sl_occ); sl_occ = sl_makeGrid(sl_H, sl_W, 0);
    sl_release_arr(sl_emi); sl_emi = sl_makeGrid(sl_H, sl_W, 0);

    while (sl_x < sl_W) {
        sl_arr_set_int(sl_arr_get(sl_occ, sl_H - 3), sl_x, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_H - 2), sl_x, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_H - 1), sl_x, 1);
        sl_x++;
    }

    while (sl_y < sl_H - 3) {
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 11, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 12, 1);
        sl_y++;
    }
    sl_y = 9;
    while (sl_y < sl_H - 3) {
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 27, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 28, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 29, 1);
        sl_y++;
    }
    sl_y = 7;
    while (sl_y < sl_H - 3) {
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 43, 1);
        sl_arr_set_int(sl_arr_get(sl_occ, sl_y), 44, 1);
        sl_y++;
    }
    sl_arr_set_dbl(sl_arr_get(sl_emi, 2), 4, (double)(1.0));
    sl_arr_set_dbl(sl_arr_get(sl_emi, 2), sl_W - 5, (double)(0.8));
    SlMap* _r = ({ SlMap* _obj_1 = sl_map_new(2);
   sl_map_set(_obj_1, "occ", sl_box_arr(sl_occ));
   sl_map_set(_obj_1, "emi", sl_box_arr(sl_emi));
   _obj_1; });
    sl_release_arr(sl_occ);
    sl_release_arr(sl_emi);
    return _r;
}
double sl_shadowFactor(SlArray* sl_occ, long long sl_x, long long sl_y, long long sl_lx, long long sl_ly) {
    long long sl_dx = 0;
    long long sl_dy = 0;
    long long sl_dist = 0;
    long long sl_steps = 0;
    long long sl_i = 1;
    long long sl_t = 0;
    long long sl_sx = 0;
    long long sl_sy = 0;
    sl_dx = sl_lx - sl_x;
    sl_dy = sl_ly - sl_y;
    sl_dist = sqrt(sl_dx * sl_dx + sl_dy * sl_dy);
    if (sl_dist < 1) {
        return 1.0;
    }
    sl_steps = (long long)floor(sl_dist * 1.2);
    if (sl_steps < 1) {
        sl_steps = 1;
    }
    ...
    while (sl_i < sl_steps) {
        sl_t = sl_i / sl_steps;
        sl_sx = (long long)floor(sl_x + sl_dx * sl_t);
        sl_sy = (long long)floor(sl_y + sl_dy * sl_t);
        if (sl_sx >= 0 && sl_sx < sl_W && sl_sy >= 0 && sl_sy < sl_H) {
            if (sl_arr_get((SlArray*)sl_arr_get(sl_occ, sl_sy), sl_sx) == 1) {
                return 0.08;
            }
        }
        sl_i++;
...
SlArray* sl_computeDirect(long long sl_scene, long long sl_lx, long long sl_ly) {
    long long sl_y = 0;
    long long sl_x = 0;
    long long sl_dx = 0;
    long long sl_dy = 0;
    long long sl_d = 0;
    long long sl_atten = 0;
    double sl_sh = 0;
    SlArray* sl_light = 0;
    SlArray* sl_occ = 0;
    SlArray* sl_emi = 0;
    sl_release_arr(sl_occ); sl_occ = sl_arr_retain(sl_map_get(sl_scene, "occ", sl_null()).aval);
    sl_release_arr(sl_emi); sl_emi = sl_arr_retain(sl_map_get(sl_scene, "emi", sl_null()).aval);
    sl_release_arr(sl_light); sl_light = sl_makeGrid(sl_H, sl_W, 0);

    while (sl_y < sl_H) {
        sl_x = 0;
        while (sl_x < sl_W) {
            if (sl_arr_get((SlArray*)sl_arr_get(sl_occ, sl_y), sl_x) == 1) {
                sl_arr_set_int(sl_arr_get(sl_light, sl_y), sl_x, 0);
            } else {
                sl_dx = sl_lx - sl_x;
                sl_dy = sl_ly - sl_y;
                sl_d = sqrt(sl_dx * sl_dx + sl_dy * sl_dy);
                sl_direct = 0;
                if (sl_d < sl_RADIUS) {
                    sl_atten = 1 - sl_d / sl_RADIUS;
                    sl_sh = sl_shadowFactor(sl_occ, sl_x, sl_y, sl_lx, sl_ly);
                    sl_direct = sl_atten * sl_atten * sl_sh;
                }
                sl_arr_set_dbl(sl_arr_get(sl_light, sl_y), sl_x, (double)(sl_direct + sl_arr_get((SlArray*)sl_arr_get(sl_emi, sl_y), sl_x) * 1.6 + 0.02));
            }
            sl_x++;
        }
        sl_y++;
...
SlArray* sl_diffuse(SlArray* sl_light, SlArray* sl_occ, long long sl_iterations) {
    long long sl_it = 0;
    long long sl_y = 0;
    long long sl_x = 0;
    long long sl_acc = 0;
    ...
    while (sl_it < sl_iterations) {
        sl_release_arr(sl_next); sl_next = sl_makeGrid(sl_H, sl_W, 0);
        sl_y = 0;
        while (sl_y < sl_H) {
            sl_x = 0;
            while (sl_x < sl_W) {
                if (sl_arr_get((SlArray*)sl_arr_get(sl_occ, sl_y), sl_x) == 1) {
                    sl_arr_set_int(sl_arr_get(sl_next, sl_y), sl_x, 0);
                } else {
                    sl_c = sl_arr_get((SlArray*)sl_arr_get(sl_light, sl_y), sl_x) * 0.35;
                    sl_acc = 0;
                    sl_n = 0;
                    sl_oy = -1;
                    while (sl_oy <= 1) {
                        ...
                        while (sl_ox <= 1) {
                            if (!sl_ox == 0 && sl_oy == 0) {
                                sl_nx = sl_x + sl_ox;
                                sl_ny = sl_y + sl_oy;
                                if (sl_nx >= 0 && sl_nx < sl_W && sl_ny >= 0 && sl_ny < sl_H) {
                                    if (sl_arr_get((SlArray*)sl_arr_get(sl_occ, sl_ny), sl_nx) == 0) {
                                        sl_acc += sl_arr_get((SlArray*)sl_arr_get(sl_light, sl_ny), sl_nx);
                                        sl_n++;
                                    }
                                }
                            }
                    ...
                    if (sl_n > 0) {
                        sl_arr_set_dbl(sl_arr_get(sl_next, sl_y), sl_x, (double)(sl_c + sl_acc / sl_n * 0.62));
                    } else {
                        sl_arr_set_dbl(sl_arr_get(sl_next, sl_y), sl_x, (double)(sl_c));
                    }
                }
                sl_x++;
            }
        ...
        sl_release_arr(sl_light); sl_light = sl_next;
        sl_it++;
    }
    SlArray* _r = sl_light;
    sl_release_arr(sl_next);
    return _r;
}
long long sl_renderFrame(long long sl_scene, SlArray* sl_light, long long sl_lx, long long sl_ly, long long sl_frameNo) {
    long long sl_maxIdx = 0;
    long long sl_y = 0;
    long long sl_x = 0;
    long long sl_v = 0;
    long long sl_idx = 0;
    ...
    sl_release_arr(sl_occ); sl_occ = sl_arr_retain(sl_map_get(sl_scene, "occ", sl_null()).aval);
    sl_release_arr(sl_emi); sl_emi = sl_arr_retain(sl_map_get(sl_scene, "emi", sl_null()).aval);
    sl_maxIdx = (long long)strlen(sl_RAMP) - 1;
    printf("%s\n", sl_strcat(sl_strcat(sl_strcat(sl_strcat(sl_strcat(sl_strcat("Frame ", sl_toString(sl_frameNo)), "  Light("), sl_toString(sl_lx)), " "), sl_toString(sl_ly)), ")"));

    while (sl_y < sl_H) {
        sl_release_str(sl_line); sl_line = strdup("");
        sl_x = 0;
        while (sl_x < sl_W) {
            if (sl_x == sl_lx && sl_y == sl_ly) {
                { char* _old = sl_line; sl_line = sl_strcat(sl_line, "@"); sl_release_str(_old); }
            } else {
                if (sl_arr_get((SlArray*)sl_arr_get(sl_emi, sl_y), sl_x) > 0.7) {
                    { char* _old = sl_line; sl_line = sl_strcat(sl_line, "*"); sl_release_str(_old); }
                } else {
                    if (sl_arr_get((SlArray*)sl_arr_get(sl_occ, sl_y), sl_x) == 1) {
                        { char* _old = sl_line; sl_line = sl_strcat(sl_line, "#"); sl_release_str(_old); }
                    } else {
                        sl_v = sl_clamp(sl_arr_get((SlArray*)sl_arr_get(sl_light, sl_y), sl_x), 0, 1);
                        sl_idx = (long long)floor(sl_v * sl_maxIdx);
                        { char* _old = sl_line; sl_line = sl_strcat(sl_line, sl_str_substring(sl_RAMP, sl_idx, sl_idx + 1)); sl_release_str(_old); }
                    }
                }
            }
            sl_x++;
    ...
    printf("%s\n", "");
    sl_release_str(sl_line);
    sl_release_arr(sl_emi);
    sl_release_arr(sl_occ);
    return 0;
}
int main(int argc, char* argv[]) {
    long long sl_FRAMES = 24;;
    long long sl_f = 0;;
    long long sl_gi = 0;;
    sl_W = 56;;
    sl_H = 22;;
    ...
    sl_ly = 0;;
    sl_direct = 0;;
    printf("%s\n", "=== SeedLang Lumen Lite: Simplified GI Demo ===");
    printf("%s\n", "Mode: CLI ASCII");
    printf("%s\n", "");
    printf("%s\n", "init scene ...");
    sl_release_map(sl_scene); sl_scene = sl_buildScene();
    printf("%s\n", "scene ok");
    while (sl_f < sl_FRAMES) {
        sl_lx = 8 + (long long)floor((sl_W - 16) * (0.5 + 0.5 * sin(sl_f * 0.35)));
        sl_ly = 4 + (long long)floor(2 * sin(sl_f * 0.21));
        sl_direct = sl_computeDirect(sl_scene, sl_lx, sl_ly);
        sl_gi = sl_diffuse(sl_direct, sl_arr_retain(sl_map_get(sl_scene, "occ", sl_null()).aval), 4);
        sl_renderFrame(sl_scene, sl_gi, sl_lx, sl_ly, sl_f + 1);
        (sl_sleep(45), 0);
        sl_f++;
    }
    printf("%s\n", "=== Done ===");
    sl_release_str(sl_RAMP);
    sl_release_map(sl_scene);
    return 0;
}