...
/** Empty `[]` literals: start as I32 so sequential int pushes (e.g. 0..1e6) skip U8→U16→I32 upgrades (each was a full copy). */
...
/** Grow backing store only; len unchanged (vector::reserve). */
...
int main(int argc, char* argv[]) {
    long long sl_i = 0;;
    sl_arr = 0;;
    { SlArray* _old = sl_arr; sl_arr = sl_arr_new_i32((int)(((long long)(1000000)) > 4LL ? ((long long)(1000000)) : 4LL)); sl_release_arr(_old); }
    { long long _wl_i = sl_i;
      while (_wl_i < 1000000) {
        (sl_arr_push_int(sl_arr, _wl_i), 0);
        _wl_i++;
      }
      sl_i = _wl_i;
    }
    printf("%lld\n", (long long)(sl_arr_len(sl_arr)));
    sl_release_arr(sl_arr);
    return 0;
}